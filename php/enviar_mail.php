   ...
   require '../../vendor/autoload.php';
      ...
      $response = [];
		...
		try{
         ...
         $result = pg_query($conn, "UPDATE usuarios_pqrs set recover = MD5(''||NOW()||'') WHERE email = '$email';");
         $select = pg_query($conn, "SELECT recover, nombre1 FROM usuarios_pqrs WHERE email = '$email';");
         $id = pg_fetch_row($select);
            ...
            $message .= "<tbody>
                  ...
                  <p style='font-size:20px;'>Hola ".$id[1].", nos has solicitado reetablecer tu contraseña haz click en este <a href='http://170.246.112.3:8091/gestion_pqrsdf/recover_pass.php?id=".$email."&code=".$id[0]."'>Link.</a></p>
                  <hr />
